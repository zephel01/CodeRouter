...
最終更新: 2026-04-20
作成者: zephel01
状態: **v0.6.0 umbrella tag 準備完了** (2026-04-20) — v0.6-A/B/C/D を一本の tag にまとめる retrospective + CHANGELOG 整備を完了。主要 deliverable: **startup fast-fail validator** (`default_profile` / `mode_aliases` target / `CODEROUTER_MODE` の 4 例で `@model_validator(mode="after")` パターンが default に昇格) と **typed log payload + chokepoint helper** (v0.6-C `ChainPaidGateBlockedPayload` は v0.5.1 A-1 の 2 例目、以降は convention 化) が minor 全体の設計 spine に。`_resolve_chain` が 4 engine entry points (generate / stream / generate_anthropic / stream_anthropic) の chokepoint として再確認 (v0.6-C warn 配置で dividend 回収)。§9.3 の v0.5 未着手分は capability mismatch → chain skip (v1.0+) 以外全消化。詳細は [`docs/retrospectives/v0.6.md`](./docs/retrospectives/v0.6.md)。計 **306 tests green** (267 → 306, +39, +15%)。直前: v0.6-D `mode_aliases` + `X-CodeRouter-Mode` header。
