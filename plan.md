...
| 領域 | 内容 | ターゲット |
| --- | --- | --- |
| **配布 — onboarding ウィザード** | `setup.sh` (RAM 検出 → 推奨ローカルモデル pull → `providers.yaml` 生成) | v1.7-B 以降 |
| **配布 — launcher 自動生成** | macOS `.command` / Linux `.sh` / Windows `.bat` を `coderouter setup` 系から生成 | v1.7-B 以降 |
| **Doctor — network audit** | `coderouter doctor --network` (`lsof -i -P` 相当 + ホワイトリスト照合 + 「localhost only」のグリーン表示) | v1.7-B 以降 |
| **Doctor — 全部回す mode** | `coderouter doctor --check-config` / `--check-adapter` / 引数なしで全 probe を順次実行 | v1.7-B 以降 |
| **起動時アップデートチェック (opt-in)** | PyPI に新版があれば INFO ログ 1 行、明示的に opt-in (デフォルト OFF) | v1.7-B 以降 |
| **`claude_code_suitability` hint** | capability registry に `degraded` フラグ、Llama-3.3-70B 系を Claude Code チェーンに置いたとき startup で WARN (v1.6.2 検証で確認した過剰ツール呼び出し罠の構造化対応) | v1.7 |
| **Trusted Publishing 自動化** | PyPI 側で「pending publisher」(GitHub Actions release.yml + environment `pypi`) を登録 → 次 tag push (v1.7.1+) で自動 publish | v1.7.1 push と同時 |
| **14 ケース回帰スイート** | claude-code-local 同等の Claude Code 互換性 e2e (tools / streaming / system prompt 多段 / cache_control / 失敗復旧) | v1.0 残スコープ |
| **Code Mode (Claude Code harness slim)** | claude-code-local の Claude Code 用最小ハーネス (システムプロンプト / tool 宣言 / SSE 整形を vendor-neutral 化) | v1.0 残スコープ |
| **`recover_garbled_tool_json` 完全版** | v1.0-A は output-cleaning のみ。tool-call 変換層の壊れ JSON 修復の範囲拡大 (現在 `translation/tool_repair.py` の balanced-brace + fenced JSON にとどまる) | v1.0 残スコープ |
| **チューニング既定値の自動推定** | doctor が探した `num_ctx` / `num_predict` / `temperature` を `providers.yaml` に書き戻す one-shot コマンド | v1.0 残スコープ |
| **Retrospective: v1.5 / v1.6 / v1.7** | `docs/retrospectives/v1.5.md` (Observability) / `v1.6.md` (auto_router + NIM + `.env` hygiene) / `v1.7.md` (PyPI publish) | 各 minor 出荷後 |
| **横断ドキュメント** | `docs/architecture.md` / `docs/providers.md` / `docs/benchmarks.md` | v2.0 までに |
| **コミュニティ** | `CONTRIBUTING.md` / ISSUE / PR テンプレート / note 記事用ネタ収集 (実測値、ハマりどころ) | v2.0 までに |
...
> **v1.7-A (PyPI 公開) は 2026-04-25 出荷済み** (§11.B.3 参照)。残りタスクは v1.7-B 以降に。
