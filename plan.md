...
最終更新: 2026-04-20
作成者: zephel01
状態: **v0.7.0 umbrella 完了** (2026-04-20) — v0.7-A / v0.7-B / v0.7-C を束ねる umbrella。narrative layer [`docs/retrospectives/v0.7.md`](./docs/retrospectives/v0.7.md) 執筆完了 — Beginner UX, made legible を theme に 3 sub-release の 3 段階 (宣言 → probe → 文書化) を narrativize、4 設計 through-line (Data-as-configuration / Diagnostic surface that bypasses runtime / Dominant-signal short-circuit with SKIP preserved / Non-code release as a sub-release boundary) を identify、8 follow-on (num_ctx probe / `--json` / CI smoke / v1.0 output-cleaning probe / real-machine verify / test-count updater / doc touchpoint automation / HF-on-Ollama glob) を記録。計 **382 tests green** (306 → 382, +76, +25%)、Runtime deps 5 → 5 維持。plan.md §9.4 DoD 全 6 項目 check、v0.7 minor は narrative + code + docs 全て closed。残は operator-side の `git tag v0.7.0` のみ。直前: v0.7-C docs loop-closure。
