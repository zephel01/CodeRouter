...
- [11. v1.6 — 配布 / launcher / doctor](#11-v16--配布--launcher--doctor)
...
- 各マイルストーン (v0.1 / v0.2 / v0.3 / v0.4 / v0.5 / v1.0 / v1.5 / v1.6 / v2.0) のスコープと完了条件を明確化する ※v1.1 は旧ラベル、v1.5 を先行出荷したため配布 / launcher / doctor ブロックは v1.6 に後送り
...
| **v1.6** | +2週 | 配布周り (uvx / launcher / doctor) — 旧 v1.1 ラベル、v1.5 先行出荷により繰り下げ | `uvx coderouter` で起動、double-click launcher、`coderouter doctor --network` で外向き 0 を表示 |
...
テスト総増分は src LOC 増分 (~+400 v1.0 まで / ~+2800 v1.5 まで) を下回るペースに。v1.5 は UI / HTML / curses driver が大部分で、unit test 化しにくい面が増えたため ratio は以前より低め。capability 系の 1 つの予期挙動につき unit / integration を揃えてきた v0.x〜v1.0 の密度は維持。
...
## 11. v1.6 — 配布 / launcher / doctor

> **注**: このセクションは当初 "v1.1" として予約されていたが、§12 (v1.5 メトリクスダッシュボード) を先行出荷した結果 **v1.6** に繰り下げた。タグ番号は v1.0.1 → **v1.5.0** → v1.6 の順。scope そのものは未変更 (launcher / doctor / network audit / Docker image / Go `coderouter-cli` 併設ハイブリッド)。
