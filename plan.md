...
最終更新: 2026-04-20
作成者: zephel01
状態: **v0.6-B 完了** (2026-04-20) — `FallbackChain` に optional `timeout_s` / `append_system_prompt` を追加、`ProviderCallOverrides` を adapter 層に導入して profile が provider 既定を置き換えられるように。engine が profile 解決時に 1 回だけ override を組み立てて chain 全体に配る方式。「`""` で provider directive を明示 clear」する非対称セマンティクスを採用。優先順位: profile 値 > provider 値 > 既定値 (置き換え)。`retry_max` は adapter-level retry 機構が未設計なため v0.6-B の scope 外 (§9.3 #4 partial)。計 **283 tests green** (+8、fallback 5 / openai_compat 3)。直前: v0.6-A `--mode` CLI + `CODEROUTER_MODE` env + 起動時 `default_profile` 検証。
...
| `ALLOW_PAID=false` の完全版 gate | v0.1 時点の "paid provider を呼ばない" は providers.yaml の chain で既に手動制御可能。宣言的 gate への昇格は v0.6-C 予定 | v0.6-C |
...
| `mode` ヘッダ優先ルーティング | 現状 header-based profile selection は動く。さらに "mode=long → 長文特化 profile" 的な mapping 層が v0.6-D 予定 | v0.6-D |
...
- [ ] **当初 v0.5 スコープの未着手分** (§9.3 参照) — `profiles.yaml` schema / ~~`--mode` CLI~~ (v0.6-A 完了) / 宣言的 `ALLOW_PAID` gate (v0.6-C 予定) / ~~プロファイル別 timeout~~ (v0.6-B 完了、retry は繰延) / mode ヘッダ優先ルーティング (v0.6-D 予定) / capability mismatch 時の chain 完全スキップ (vision 系は v1.0+)。
