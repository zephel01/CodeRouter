...
最終更新: 2026-04-20
作成者: zephel01
状態: **v0.6-D 完了** (2026-04-20) — `mode_aliases` YAML block + `X-CodeRouter-Mode` header で「intent と implementation の名前空間を分離」。client は `coding` / `long` / `fast` のような**意図名**を送れば、router が裏で aliased profile に解決する (`mode-alias-resolved` INFO log 付き)。precedence: body `profile` > `X-CodeRouter-Profile` > `X-CodeRouter-Mode` > `default_profile` — Mode は明示 Profile に負ける (proxy による意図混入の事故を回避)。起動時 `_check_mode_alias_targets_exist` validator が broken alias を fast-fail。OpenAI / Anthropic 両 ingress 対称。計 **306 tests green** (+15、schema 3 / OpenAI ingress 6 / Anthropic ingress 6)。直前: v0.6-C 宣言的 `ALLOW_PAID` gate + `chain-paid-gate-blocked` 集約 warn。
