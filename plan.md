...
最終更新: 2026-04-20
作成者: zephel01
状態: **v0.7-A 完了** (2026-04-20) — 宣言的 `model-capabilities.yaml` registry で v0.5-A の Python literal regex heuristic を外出し。bundled default (`coderouter/data/model-capabilities.yaml`) + user override (`~/.coderouter/model-capabilities.yaml`) の 2 層、`first-match-per-flag` semantics、glob + kind filter。`provider_supports_thinking` の判定は v0.5-A と 1:1 維持、振る舞い変更ゼロ。Anthropic 新 family 追加は YAML の 1 行 edit で完結し release cycle の遅延が解消。providers.yaml `capabilities.*` の explicit opt-in は最優先のまま。plan.md §9.4 の v0.7 scope で v0.7-A のみ消化 (v0.7-B doctor probe / v0.7-C Troubleshooting が残)。計 **345 tests green** (306 → 345, +39, +13%、`tests/test_capability_registry.py` 新規)。直前: v0.6.0 umbrella tag (v0.6-A/B/C/D)。
...
振り返り: [`docs/retrospectives/v0.4.md`](./docs/retrospectives/v0.4.md) / [`docs/retrospectives/v0.5.md`](./docs/retrospectives/v0.5.md) / [`docs/retrospectives/v0.5-verify.md`](./docs/retrospectives/v0.5-verify.md)。
...
- [11. v1.1 — 配布 / launcher / doctor](#11-v11--配布--launcher--doctor)
- [12. v1.5 — 計測ダッシュボード](#12-v15--計測ダッシュボード)
...
- 各マイルストーン (v0.1 / v0.2 / v0.3 / v0.4 / v0.5 / v1.0 / v1.1 / v1.5 / v2.0) のスコープと完了条件を明確化する
...
テスト総増分は src LOC 増分 (~+400) の約 1.6 倍。capability 系の 1 つの予期挙動につき unit / integration を揃えてきた結果。
...
- [ ] `coderouter doctor --check-model <provider>` が 5 症状のうち 3 + 4 + 5 を検出可能 (1 の num_ctx / 2 の tool_calls / 3 の thinking は該当 probe で必ずカバー)
- [ ] Registry 乖離時の suggested patch が `providers.yaml` / `model-capabilities.yaml` どちらにも copy-paste 可能
- [ ] 終了コード `0` / `1` / `2` が CI で流用可能 (smoke test シナリオ 1 本添付)
- [ ] README Troubleshooting に 5 症状全て記述、各症状に probe コマンド導線
- [ ] v0.7.0 umbrella tag + [`docs/retrospectives/v0.7.md`](./docs/retrospectives/v0.7.md) 執筆
- [x] テスト合計 306 → ≈340 target (+~35: v0.7-A +15 / v0.7-B +20) — **v0.7-A で 306 → 345 (+39)**、`tests/test_capability_registry.py` 新規。v0.7-A 単独で target 範囲を超過、v0.7-B ぶんは純増
