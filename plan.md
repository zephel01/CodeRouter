...
| **v1.7** | +2週 | 配布周り (uvx / launcher / doctor) — 旧 v1.1 ラベル、v1.5 → v1.6 先行出荷により繰り下げ | `uvx coderouter` で起動、double-click launcher、`coderouter doctor --network` で外向き 0 を表示 |
...
> **状態**: 起票済み (2026-04-22)、仕様ドラフト → テスト → 実装の順で数日スプリント。
...
- [ ] `docs/quickstart.md` (英語版、#58) — 別タスクで対応
- [ ] README / README.ja フロント面への `auto_router` 記載 — 次の doc pass に送り (既存 v1.5 observability 記載で当面は足りる判断)
