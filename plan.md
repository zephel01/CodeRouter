...
最終更新: 2026-04-20
作成者: zephel01
状態: **v0.6-A 完了** (2026-04-20) — `--mode <profile>` CLI + `CODEROUTER_MODE` env var + 起動時の `default_profile` 存在検証を追加。サーバー起動時点の profile 選択が 1 級市民に昇格し、`default_profile:` を書き換えずに軽く override できるように。優先順位: request per-call > `--mode` > `CODEROUTER_MODE` env > YAML `default_profile`。未知 profile は起動時 fast-fail (従来は first-request で 500)。計 **275 tests green** (+8)。直前: v0.5-D OpenRouter roster 週次 cron (`scripts/openrouter_roster_diff.py`)。
...
| プロファイル別 timeout / retry | provider-level で timeout/retryable は既に制御可能。profile-level の override は v0.6-B 予定 | v0.6-B |
