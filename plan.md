...
**v1.7-B umbrella (推奨実施順、詳細は §11.B.4)**:

| # | 領域 | 内容 | 工数 |
| --- | --- | --- | --- |
| 1 | **Trusted Publishing 自動化** | PyPI 側で pending publisher を登録 → 次 tag push (v1.7.1+) で自動 publish (作業 5 分、コード変更ゼロ) | 5 分 |
| 2 | **`claude_code_suitability` hint** | capability registry に `degraded` フラグ、Llama-3.3-70B 系を `claude-code` 系プロファイルに置いたとき startup で WARN (v1.6.2 で docs 化した過剰ツール呼び出し罠の構造化対応) | 1 日 |
| 3 | **`coderouter doctor --check-model --apply`** | doctor の YAML パッチを `providers.yaml` に非破壊書き戻し (key order + コメント保持)、`--dry-run` で diff プレビュー | 1〜2 日 |
| 4 | **`setup.sh` (onboarding ウィザード)** | RAM 検出 → 推奨ローカルモデル提案 → `ollama pull` → `providers.yaml` template 生成 (新規依存ゼロ、bash + 既存 doctor) | 1.5〜2 日 |
