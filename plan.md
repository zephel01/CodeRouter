...
最終更新: 2026-04-20
作成者: zephel01
状態: **v0.6-C 完了** (2026-04-20) — 宣言的 `ALLOW_PAID` gate の強化。`_resolve_chain` が paid-blocked provider 名を収集し、chain が empty になった瞬間に集約 warn `chain-paid-gate-blocked` を 1 行吐く (`ChainPaidGateBlockedPayload` TypedDict + `log_chain_paid_gate_blocked` chokepoint helper、v0.5.1 A-1 の `CapabilityDegradedPayload` パターン踏襲)。既存 per-provider INFO `skip-paid-provider` / `NoProvidersAvailableError` 例外 shape は非破壊。`_resolve_chain` 一本化により 4 entry points (generate / stream / generate_anthropic / stream_anthropic) 全てで発火。計 **291 tests green** (+8、`tests/test_fallback_paid_gate.py` 新規)。直前: v0.6-B `FallbackChain.timeout_s` / `append_system_prompt` + `ProviderCallOverrides`。
...
- [ ] **当初 v0.5 スコープの未着手分** (§9.3 参照) — `profiles.yaml` schema / ~~`--mode` CLI~~ (v0.6-A 完了) / ~~宣言的 `ALLOW_PAID` gate~~ (v0.6-C 完了) / ~~プロファイル別 timeout~~ (v0.6-B 完了、retry は繰延) / mode ヘッダ優先ルーティング (v0.6-D 予定) / capability mismatch 時の chain 完全スキップ (vision 系は v1.0+)。
