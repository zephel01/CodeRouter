...
- Docker イメージは未提供 — `plan.md §11.B` が v1.7 以降で追跡（当初 v1.1 → v1.6 と繰り下げ。v1.6 は `auto_router` / NIM / `--env-file` を先行出荷、配布周りは v1.7-A の PyPI 公開でまず onboarding 摩擦を解消し、Docker / launcher は v1.7-B 以降）。
